...
>(({ className, variant = 'primary', ...props }, ref) => {
  const variants = {
    ...
    outline: 'bg-transparent border border-border text-text-muted hover:bg-surface',
  };
  return (
    <button
      ref={ref}
      className={cn(
        'inline-flex items-center justify-center rounded-lg px-4 py-2 text-sm font-semibold transition-all active:scale-95 focus:outline-none focus:ring-2 focus:ring-primary/40 disabled:opacity-50 disabled:pointer-events-none',
        variants[variant],
        className
      )}
      {...props}
